...
pub fn enable(env: JNIEnv, _obj: JObject) {
    ...
    unsafe { RUST_BUKKIT = Some(RustBukkit::new()) }
}
...
pub(crate) fn get_bukkit() -> &'static mut RustBukkit {
    unsafe {
        RUST_BUKKIT
            .as_mut()
            .expect("get_instance called before enable!")
    }
}

pub(crate) fn get_event_handler<'a>(env: JNIEnv<'a>, obj: JObject<'a>) -> JObject<'a> {
    let rust_bukkit = env
        .get_field(obj, "rustBukkit", "Lnet/insprill/rustbukkit/RustBukkit;")
        .expect("Failed to get rust bukkit")
        .l()
        .expect("Failed to get object");
    env.get_field(
        rust_bukkit,
        "eventHandler",
        "Lnet/insprill/rustbukkit/event/RustEventHandler;",
    )
    .expect("Failed to get event handler")
    .l()
    .expect("Failed to get object")
}
