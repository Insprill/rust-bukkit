...
#[no_mangle]
pub extern "system" fn Java_net_insprill_rustbukkit_command_RustCommandHandler_execute(
    env: JNIEnv,
    _obj: JObject,
    j_name: JString,
    j_label: JString,
...
#[no_mangle]
pub extern "system" fn Java_net_insprill_rustbukkit_command_RustCommandHandler_tabComplete(
    env: JNIEnv,
    _obj: JObject,
    j_name: JString,
    j_label: JString,
    ...
    for command in &get_bukkit().commands {
        ...
        if let Some(completer) = &command.tab_completer {
            ...
            let arr = env
                .new_object_array(args.len() as i32, "java/lang/String", JObject::null())
                .expect("Failed to create arg array");
            for (idx, arg) in args.iter().enumerate() {
                let arg = env
                    .new_string(arg)
                    .expect("Failed to create java string from arg");
                env.set_object_array_element(arr, idx as i32, arg)
    ...
    env.new_object_array(0, "java/lang/String", JObject::null())
        .expect("Failed to create empty arg array")
}
...
fn parse_command_args(
    ...
    let label: String = env
        .get_string(j_label)
        .expect("Failed to get command label!")
        .into();
    let len = env
        .get_array_length(j_args)
        .expect("Failed to get args length!");
    let mut args: Vec<String> = Vec::new();
    for x in 0..len {
        let element = env
            .get_object_array_element(j_args, x)
            .expect("Failed to get array element");
        args.push(
            env.get_string(JString::from(element))
                .expect("Failed to get arg string!")
                .into(),
        )
    }
    CommandArgs { name, label, args }
}
